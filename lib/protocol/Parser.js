  ...
  function parseGeometry() {
    ...
    switch(wkbType) {
      case 1: // WKBPoint
        ...
        result = {x: x, y: y, geoType: "POINT", stringValue: "POINT(" + x + " " + y + ")"};
        break;
      case 2: // WKBLineString
        var numPoints = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        result = { geoType: "LINESTRING", values: [], stringValue: ""};
        var lineStringArrayVal = [];
        for(var i = numPoints; i > 0; i--) {
          var x = byteOrder ? buffer.readDoubleLE(offset) : buffer.readDoubleBE(offset); offset += 8;
          var y = byteOrder ? buffer.readDoubleLE(offset) : buffer.readDoubleBE(offset); offset += 8;
          lineStringArrayVal.push(x + " " + y);
          result.values.push({x: x, y: y});
        }
        result.stringValue = "LINESTRING(" +lineStringArrayVal.join(", ") +")";
        break;
      case 3: // WKBPolygon
        var numRings = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        result = { geoType: "POLYGON", values: [], stringValue: ""};
        var polygonArrayVal = [];
        for(var i = numRings; i > 0; i--) {
          ...
          for(var j = numPoints; j > 0; j--) {
            var x = byteOrder ? buffer.readDoubleLE(offset) : buffer.readDoubleBE(offset); offset += 8;
            var y = byteOrder ? buffer.readDoubleLE(offset) : buffer.readDoubleBE(offset); offset += 8;
            pointArrayVal.push(x + " " + y);
            line.push({x: x, y: y});
          }
          polygonArrayVal.push("(" + pointArrayVal.join(", ") + ")");
          result.values.push(line);
        }
        result.stringValue = "POLYGON(" + polygonArrayVal.join(", ") + ")";
        break;
      case 4: // WKBMultiPoint
        var num = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        var result = { geoType: "MULTIPOINT", values: [], stringValue: ""};
        var multiArrayVal = [];
        for(var i = num; i > 0; i--) {
          result.values.push(parseGeometry());
        }
        for (var mIndex = 0; mIndex < result.values.length; mIndex++) {
          var multi = result.values[mIndex];
          var multiString = multi.stringValue ? multi.stringValue : null;
          multiArrayVal.push(multiString);
        }
        result.stringValue = "MULTIPOINT(" + multiArrayVal.join(", ") + ")";
        break;
      case 5: // WKBMultiLineString
        var num = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        var result = { geoType: "MULTILINESTRING", values: [], stringValue: ""};
        var multiArrayVal = [];
        for(var i = num; i > 0; i--) {
          result.values.push(parseGeometry());
        }
        ...
        result.stringValue = "MULTILINESTRING(" + multiArrayVal.join(", ") + ")";
      case 6: // WKBMultiPolygon
        var num = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        var result = { geoType: "MULTIPOLYGON", values: [], stringValue: ""};
        var multiArrayVal = [];
        for(var i = num; i > 0; i--) {
          result.values.push(parseGeometry());
        }
        ...
        result.stringValue = "MULTIPOLYGON(" + multiArrayVal.join(", ") + ")";
      case 7: // WKBGeometryCollection
        var num = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        var result = { geoType: "GEOMETRYCOLLECTION", values: [], stringValue: ""};
        var multiArrayVal = [];
        for(var i = num; i > 0; i--) {
          result.values.push(parseGeometry());
        }
        ...
        result.stringValue = "GEOMETRYCOLLECTION(" + multiArrayVal.join(", ") + ")";
    }
    return result;
  }
