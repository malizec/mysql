  ...
  function parseGeometry() {
    ...
    switch(wkbType) {
      case 1: // WKBPoint
        ...
        result = {x: x, y: y, geoType: "POINT"};
        break;
      case 2: // WKBLineString
        var numPoints = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        result = { geoType: "LINESTRING", value: []};
        for(var i = numPoints; i > 0; i--) {
          var x = byteOrder ? buffer.readDoubleLE(offset) : buffer.readDoubleBE(offset); offset += 8;
          var y = byteOrder ? buffer.readDoubleLE(offset) : buffer.readDoubleBE(offset); offset += 8;
          result.value.push({x: x, y: y});
        }
        break;
      case 3: // WKBPolygon
        var numRings = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        result = { geoType: "POLYGON", value: []};
      ...
      case 4: // WKBMultiPoint
        ...
        var result = { geoType: "MULTIPOINT", value: []};
        for(var i = num; i > 0; i--) {
          result.value.push(parseGeometry());
        }
        break;
      case 5: // WKBMultiLineString
        var num = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        var result = { geoType: "MULTILINESTRING", value: []};
        for(var i = num; i > 0; i--) {
          result.value.push(parseGeometry());
        }
        break;
      case 6: // WKBMultiPolygon
        var num = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        var result = { geoType: "MULTIPOLYGON", value: []};
        for(var i = num; i > 0; i--) {
          result.value.push(parseGeometry());
        }
        break;
      case 7: // WKBGeometryCollection
        var num = byteOrder ? buffer.readUInt32LE(offset) : buffer.readUInt32BE(offset); offset += 4;
        var result = { geoType: "GEOMETRYCOLLECTION", value: []};
        for(var i = num; i > 0; i--) {
          result.value.push(parseGeometry());
        }
        break;
